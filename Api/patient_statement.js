...
$("#printt_statement").on("click", function(){
  let file= new Blob([$('#printt_Area').html()], {type:"application.pdf"});
  let url= URL.createObjectURL(file);
  let a= $("<a />", {
      href: url,
      download: "print_statement.pdf"}).appendTo("body").get(0).click();
      e.preventDefault();
...
$("#patienttform").on("submit", function(event){
    ...
    let sendingData = {
         ...
         "from" : from,
         "to" : to,
        "action": "read_all_patient_statement",
  ...
  $.ajax({
    ...
    success: function(data){
        ...
        if(status){
            ...
            $("#patienTable thead").append(th);
            $("#patienTable tbody").append(tr);
        }
